...
# All of the known dictionary keys: from docassemble/base/parse.py:2094, in Question.__init__
all_dict_keys = ('features', 'scan for variables', 'only sets', 'question', 'code', 'event',
 ...
 'action', 'backgroundresponse', 'response', 'binaryresponse', 'all_variables',
 'response filename', 'content type', 'redirect url', 'null response', 'sleep',
 'include_internal', 'css class', 'table css class', 'response code',
 'subquestion', 'reload', 'help', 'audio', 'video', 'decoration', 'signature',
 'under', 'pre', 'post', 'right', 'check in', 'yesno', 'noyes', 'yesnomaybe',
 'noyesmaybe', 'sets', 'event', 'choices', 'buttons', 'dropdown', 'combobox',
 'field', 'shuffle', 'review', 'need', 'depends on', 'target', 'table', 'rows',
 'columns', 'require gathered', 'allow reordering', 'edit', 'delete buttons',
 'confirm', 'read only', 'edit header', 'confirm', 'show if empty', 'template',
 'content file', 'content', 'subject', 'reconsider', 'undefine',
 'continue button field', 'fields', 'indent', 'url', 'default', 'datatype',
 'extras', 'allowed to set', 'show incomplete', 'not available label',
 'required', 'always include editable files', 'question metadata',
 'include attachment notice', 'include download tab', 'manual attachment list',
 'breadcrumb', 'tabular', 'hide continue button', 'disable continue button',
 'pen color')


class YAMLError:
  def __init__(self, *, err_str: str, line_number: int, file_name: str, experimental: bool = True):
...
def find_errors(input_file):
  ...
  for source_code in document_match.split(full_content):
    ...
    try:
      ...
      all_errors.append(YAMLError(err_str=errMess, line_number=line_number, file_name=input_file, experimental=False))
    ...
    if len(any_types) == 0:
      all_errors.append(YAMLError(err_str=f"No possible types found: {doc}",
    ...
    for key in doc.keys():
      if key in big_dict and 'type' in big_dict[key]:
        ...
        for err in test.errors:
          all_errors.append(YAMLError(err_str=f"{err[0]}", line_number=err[1] + doc["__line__"] + line_number, file_name=input_file))
    line_number += lines_in_code
  return all_errors
